...
Once a message has arrived at an MDA, the recipient's MUA must retrieve the message. For this, most MUAs use either POP or IMAP. Post Office Protocol (POP) is a simple protocol that enables downloads and deletions, and is most often used when a single client is interested in retrieving mail from an MDA \cite{reynolds1984post}, \cite{myers1996post}. The Internet Message Access Protocol is a more complex protocol that enables multiple clients to be connected to the same mailbox, and has come into more widespread use in recent years \cite{crispin2003internet}.
...
While anonymous servers abound, mail clients are generally left to the user's preference. Yet one mail client solution comes in to save the day! Mailpile, rescuing users from the humdrum toil of non-anonymous, unprotected existence \cite{mailpile}! Mailpile is The Usability Solution, making PGP as easy to use as the little lock icon in https. But that's not all! With this one neat experimental plugin trick (SMTorP), a Mailpile user can send their emails over Tor \cite{smtorp}! And it works just like a regular email, except that it's sent to a .onion address instead! Wow! A .onion address is a Tor hidden service, which they do admit is a security liability since it essentially forces any mail client who wants to receive SMTorP emails to run a Tor hidden service on their machine, opening them up to all sorts of fun weird incoming traffic! It uses hashcash to keep the spam (bulk commercial unsolicited email) out, and users could advertise their addresses in the headers of legacy emails that they send, as an upgrade rollout strategy!
...
encrypt, hiding sender information inside the contents, inside a special folder. index, lock messages to handle multiple clients. hide metadata by using a combination of scheduling, mixing, fake decoy messages, and possibly ORAM-like techniques. rollback detection for index and messages. key is passphrase/scrypt kdf with salt.
...
\subsection{Timing}
time of day / week data, distribution of interarrival times
